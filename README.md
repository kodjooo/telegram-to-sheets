...
Автоматизированная система для обработки логов ошибок из Telegram канала и их анализа с последующей записью в Google Sheets. Скрипты собирают и группируют логи (нормализация + склейка разрезанных сообщений), а анализ выполняет ежедневная агентная рутина (Claude Code, локально у владельца): она проставляет вердикты по группам и пишет дайджест дня, который скрипт отправляет в Telegram.
...
3. **Ежедневная агентная рутина** (Claude Code, локальная задача
   `log-triage-daily`, ~09:05 МСК; инструкция — вне репозитория)
